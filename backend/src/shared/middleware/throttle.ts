...
import { whatsAppMessageThrottle } from '../../lib/add-whatsapp-message-throttling';
...
export async function throttleCheck(
  ...
  if (!orgId) {
    return; // No org context, skip throttle
  }
  ...
  if (!isMessageSend) {
    return; // Not a messaging operation, skip throttle
  }
