...
export async function orgGuard(
  ...
  // Public routes that skip org guard
  const publicRoutes = [
    '/health',
    '/metrics',
    '/webhooks/evolution',
  ...
  if (isPublic) {
    return; // Skip org guard for public routes
  }
  ...
  if (!orgId) {
    throw new Error('Bad Request: No orgId specified');
  }
...
export function isSuperAdmin(request: FastifyRequest): boolean {
  return (request as any).user?.role === 'SUPER_ADMIN';
}
