...
export async function registerMessagesRoutes(fastify: FastifyInstance) {
  // POST /:instanceId/send
  fastify.post('/:instanceId/send', async (request: FastifyRequest, reply: FastifyReply) => {
    try {
  ...
  console.log('[Messages] POST /api/v1/messages/:instanceId/send - registered');
}
